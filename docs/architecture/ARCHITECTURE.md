...
1. User runs `rosactl cluster-vpc create my-cluster` (region resolved from `--region` flag or inferred from the URL stored by `rosactl login`)
...
1. User runs `rosactl cluster-iam delete my-cluster` (region resolved from `--region` flag or inferred from the URL stored by `rosactl login`)
